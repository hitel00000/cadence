...
* **서비스의 재정의**: `기타리스트를 위한 3분 코드 요리`나 `재즈 피아노 보이싱` 책에 나오는 코드 조각들을 모바일에서 바로 들어보고, 원하는 키로 즉시 전조하여 템포별로 반복 연습할 수 있는 최적의 모바일 UX를 제공합니다.
...
### 3.1. 정적 패턴 데이터 모델 (예: `static/books/three-minute-chord-cooking.json`)
도수 정보(`degrees`)와 기본 키 코드(`chords`)를 함께 보관하여 완벽한 동적 전조를 지원합니다.

```json
{
  "bookId": "three-minute-chord-cooking",
  "bookTitle": "기타리스트를 위한 3분 코드 요리",
  "patterns": [
    {
      "id": "three-min-01",
      "title": "밝은 코드진행 (A 멜로디-01)",
      "category": "A 멜로디",
      "defaultKey": "C",
      "meter": "4/4",
